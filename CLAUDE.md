...
- `tools/generate-watchface.tsx` — `bun tools/generate-watchface.tsx` (no args) packs every preset into `app/src/main/res/raw/watchface.xml` as a WFF `ListConfiguration` style picker (long-press face → customize on the watch) and also generates `res/values/presets_strings.xml` (editor labels, do not hand-edit). All preset fields are ported except `sharp` (WFF version renders square ends always); `fontFamily`/`fontWeight` go through the `FONT_FAMILY`/`FONT_WEIGHT` maps onto bundled fonts (see `res/font/` below). Each preset with `complication: true` gets its own preset-styled round WFF `ComplicationSlot` (default day+date, user-repointable; WFF caps slots at 8 total) activated via `complicationSlotIds` on its `ListOption`. The scene is written as JSX — see below.
...
- `app/src/main/res/font/` — the six fonts a preset's `fontFamily` token can name (`sans`/`serif`/`didone`/`mono`/`condensed`/`geometric`). Each is one OFL variable TTF subset to Latin + digits, plus a generated `<name>_family.xml` that declares weights 100–900 against the same file via `fontVariationSettings` — that XML is what WFF's `Font/@family` points at, so `Font/@weight` resolves to a real weight instead of synthetic bolding. `demo.html` `@font-face`s the same TTFs so the preview matches the watch (needs http, not `file://`). The token→font mapping lives twice: `FONT_FAMILY` in the generator and `FONT_STACK` in `demo.html`.
