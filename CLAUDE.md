...
- `tools/generate-watchface.tsx` — `bun tools/generate-watchface.tsx` (no args) packs every preset into `app/src/main/res/raw/watchface.xml` as a WFF `ListConfiguration` style picker (long-press face → customize on the watch) and also generates `res/values/presets_strings.xml` (editor labels, do not hand-edit). All preset fields are ported except `sharp` (WFF version renders square ends always); `fontFamily`/`fontWeight` go through the the `FONT_FAMILY` map onto bundled fonts (see `tools/build-fonts.ts` below). Each preset with `complication: true` gets its own preset-styled round WFF `ComplicationSlot` (default day+date, user-repointable; WFF caps slots at 8 total) activated via `complicationSlotIds` on its `ListOption`. The scene is written as JSX — see below.
