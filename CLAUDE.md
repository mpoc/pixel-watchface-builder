...
- `tools/generate-watchface.ts` — `bun tools/generate-watchface.ts <preset>` writes `app/src/main/res/raw/watchface.xml`; `--all` packs every preset into one XML as a WFF `ListConfiguration` style picker (long-press face → customize on the watch) and also generates `res/values/presets_strings.xml` (editor labels, do not hand-edit). All preset fields are ported except `sharp` (WFF version renders square ends always). Presets with `complication: true` get a round bottom-center WFF `ComplicationSlot` (default day+date, user-repointable); in `--all` mode each enabled preset gets its own preset-styled slot (WFF caps slots at 8 total) activated via `complicationSlotIds` on its `ListOption`.
...
```sh
bun tools/generate-watchface.ts <preset>   # or --all for the style picker
