...
//
// Usage:
//   bun tools/generate-watchface.tsx [preset]   single preset (default: classic)
//   bun tools/generate-watchface.tsx --all      all presets as a style picker
...
const doc = [
  <WatchFace width={SCREEN} height={SCREEN}>
    ...
    {all ? (
      ...
      <Scene backgroundColor={wff(PRESETS[arg].background)}>
        {buildPreset(arg, PRESETS[arg])}
        {PRESETS[arg].complication &&
          buildComplicationSlot(arg, PRESETS[arg], 0)}
      </Scene>
    )}
  </WatchFace>,
];
...
if (all) {
  ...
  await Bun.write(
    ...
    [
      `<?xml version="1.0" encoding="utf-8"?>`,
      `<!-- Generated by tools/generate-watchface.tsx (all mode). Do not edit by hand. -->`,
      `<resources>`,
...
console.log(
  `wrote ${out} ${all ? `with all ${names.length} presets` : `from preset "${arg}"`}`,
);
