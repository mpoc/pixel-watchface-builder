...
const PRESETS = JSON.parse(presetsSrc.slice(presetsSrc.indexOf("{"), presetsSrc.lastIndexOf("}") + 1));
...
function emitPreset(name: string, p: any, emit: (s: string) => void, pad: string) {
  ...
  const NUM_R = R + (p.numeralSide === "out" ? p.numeralInset : -p.numeralInset);

  emit(`${pad}<PartDraw x="0" y="0" width="${SCREEN}" height="${SCREEN}">`);
  emit(`${pad}    <Rectangle x="0" y="0" width="${SCREEN}" height="${SCREEN}"><Fill color="${BG_COLOR}" /></Rectangle>`);
  emit(`${pad}</PartDraw>`);
  ...
  if (ROTATE) {
    emit(`${pad}<Group name="camera_${name}" x="${SCREEN / 2 - CX}" y="${round(SCREEN / 2 - CX + DIST)}" width="${BOX}" height="${BOX}" pivotX="0.5" pivotY="0.5">`);
    emit(`${pad}    <Transform target="angle" value="360.0 - (${HOUR_DEG})" />`);
  } else {
    emit(`${pad}<Group name="camera_${name}" x="0" y="0" width="${BOX}" height="${BOX}">`);
    emit(`${pad}    <Transform target="x" value="${SCREEN / 2 - CX} - sin(rad(${HOUR_DEG})) * ${DIST}" />`);
    emit(`${pad}    <Transform target="y" value="${SCREEN / 2 - CX} + cos(rad(${HOUR_DEG})) * ${DIST}" />`);
  }
  ...
  const tick = (deg: number, w: number, h: number, alpha: number) => {
    ...
    emit(`${pad}    <PartDraw x="${round(cx - w / 2)}" y="${round(cy - h / 2)}" width="${ceil(w)}" height="${ceil(h)}" angle="${deg}" pivotX="0.5" pivotY="0.5" alpha="${alpha}">`);
    emit(`${pad}        <Rectangle x="0" y="0" width="${ceil(w)}" height="${ceil(h)}"><Fill color="${DIAL_COLOR}" /></Rectangle>`);
    emit(`${pad}    </PartDraw>`);
  };

  // Hour markers (base 6px wide, 44px long).
  for (let h = 0; h < 12; h++) tick(h * 30, p.hourWeight * 6, p.hourLength * 44, 255);
  ...
  if (step > 0) {
    for (let h = 0; h < 12; h++) {
      for (let m = step; m < 60; m += step) {
        ...
        const alpha = Math.round(255 * (half ? 0.9 : fifteen ? 0.7 : five ? 0.5 : 0.27));
  ...
  for (let h = 0; h < 12; h++) {
    ...
    const bw = 120, bh = Math.max(48, FONT_SIZE + 16);
    const glued = !p.numeralsUpright;
    const angleAttr = glued ? ` angle="${degNum}" pivotX="0.5" pivotY="0.5"` : "";
    emit(`${pad}    <PartText x="${round(cx - bw / 2)}" y="${round(cy - bh / 2)}" width="${bw}" height="${bh}"${angleAttr}>`);
    if (!glued && ROTATE) emit(`${pad}        <Transform target="angle" value="${HOUR_DEG}" />`);
    emit(`${pad}        <Text align="CENTER" ellipsis="false">`);
    emit(`${pad}            <Font family="SYNC_TO_DEVICE" size="${FONT_SIZE}" weight="BOLD" color="${DIAL_COLOR}">${h === 0 ? 12 : h}</Font>`);
    emit(`${pad}        </Text>`);
    emit(`${pad}    </PartText>`);
  }
  ...
  const hand = (tag: string, w: number, angleExpr: string, color: string) => {
    const hw = ceil(w);
    emit(`${pad}<Group name="${tag}_${name}" x="0" y="0" width="${BOX}" height="${BOX}" pivotX="0.5" pivotY="0.5">`);
    emit(`${pad}    <Transform target="angle" value="${angleExpr}" />`);
    emit(`${pad}    <PartDraw x="${round(CX - hw / 2)}" y="${CX - HAND_LEN}" width="${hw}" height="${HAND_LEN + 8}">`);
    emit(`${pad}        <Rectangle x="0" y="0" width="${hw}" height="${HAND_LEN + 8}"><Fill color="${color}" /></Rectangle>`);
    emit(`${pad}    </PartDraw>`);
    emit(`${pad}</Group>`);
  };
...
function emitComplicationSlot(name: string, p: any, emit: (s: string) => void, pad: string, slotId: number) {
  const DIAL_COLOR = wff(p.dialColor);
  ...
  const line = (y: number, h: number, fontSize: number, expr: string) => {
    emit(`${pad}        <PartText x="6" y="${y}" width="${d - 12}" height="${h}">`);
    emit(`${pad}            <Text align="CENTER" ellipsis="TRUE">`);
    emit(`${pad}                <Font family="SYNC_TO_DEVICE" size="${fontSize}" weight="BOLD" color="${DIAL_COLOR}">`);
    emit(`${pad}                    <Template>%s<Parameter expression="${expr}" /></Template>`);
    emit(`${pad}                </Font>`);
    emit(`${pad}            </Text>`);
    emit(`${pad}        </PartText>`);
  };
  emit(`${pad}<ComplicationSlot slotId="${slotId}" name="date_${name}" x="${round(cx - d / 2)}" y="${round(cy - d / 2)}" width="${d}" height="${d}" supportedTypes="SHORT_TEXT" tintColor="${DIAL_COLOR}">`);
  emit(`${pad}    <DefaultProviderPolicy defaultSystemProvider="DAY_AND_DATE" defaultSystemProviderType="SHORT_TEXT" />`);
  ...
  emit(`${pad}            <Ellipse x="1" y="1" width="${d - 2}" height="${d - 2}"><Stroke color="${OUTLINE}" thickness="2" /></Ellipse>`);
  emit(`${pad}        </PartDraw>`);
  ...
  emit(`${pad}        <PartImage x="${d / 2 - 11}" y="10" width="22" height="22">`);
  emit(`${pad}            <Image resource="[COMPLICATION.MONOCHROMATIC_IMAGE]" />`);
  emit(`${pad}        </PartImage>`);
...
const all = arg === "--all";
if (!all && !PRESETS[arg]) {
  console.error(`unknown preset "${arg}"; have: --all, ${Object.keys(PRESETS).join(", ")}`);
  process.exit(1);
}
...
if (all) {
  ...
  if (compNames.length > 8) console.warn(`warning: ${compNames.length} presets have complications; only the first 8 get a slot`);
  const slotOf = new Map(compNames.slice(0, 8).map((n, i) => [n, i]));
  emit(`    <UserConfigurations>`);
  emit(`        <ListConfiguration id="preset" displayName="@string/preset_label" defaultValue="${names[0]}">`);
  for (const n of names) {
    const slot = slotOf.get(n);
    emit(`            <ListOption id="${n}" displayName="@string/preset_${n}" complicationSlotIds="${slot ?? ""}" />`);
  }
  ...
  emit(`        </ListConfiguration>`);
  for (const [n, slot] of slotOf) emitComplicationSlot(n, PRESETS[n], emit, "        ", slot);
  emit(`    </Scene>`);
  emit(`</WatchFace>`);
  ...
  await Bun.write("app/src/main/res/values/presets_strings.xml", strings.join("\n") + "\n");
} else {
  const p = PRESETS[arg];
...
console.log(`wrote ${out} ${all ? `with all ${Object.keys(PRESETS).length} presets` : `from preset "${arg}"`} (${lines.length} lines)`);
